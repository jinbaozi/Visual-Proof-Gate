...
Visual Proof Gate
  ↓ evidence + diagnosis + enhancement plan + optional safe patch + visual delta + Impeccable handoff
Impeccable Skill
...
8. **Visual delta**: `src/delta/` links before and after evidence into reports without enforcing pixel thresholds in v1.
9. **Reports, routing, and handoff**: `src/reports/` renders Markdown/JSON content; `src/routing/` maps defects to Impeccable commands; `src/handoff/` prepares Impeccable handoff context.
10. **Orchestration**: `src/orchestrator/` composes explicit stages. It does not contain DOM probe internals.
11. **Architecture guardrails**: `tests/architecture/` enforces dependency boundaries in CI.
...
The practical rule is simple: lower layers can be read without opening higher layers. Higher layers compose lower layers but do not hide probe, diagnosis, enhancement, patch, delta, report, or handoff logic inside the orchestrator.
